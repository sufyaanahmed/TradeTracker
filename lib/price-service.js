...
// Routes to NSE India for Indian stocks, Alpha Vantage for others
// Designed with a pluggable interface for easy API switching

const NSE_BASE = 'https://www.nseindia.com';
const ALPHA_VANTAGE_BASE = 'https://www.alphavantage.co/query';

// In-memory price cache (TTL: 1 hour to minimize API calls)
const priceCache = new Map();
const PRICE_CACHE_TTL = 60 * 60 * 1000; // 1 hour cache

/**
 * Detect if symbol is an Indian stock (NSE/BSE)
 * Indian stocks typically have no dots and are uppercase
 */
function isIndianStock(symbol) {
  // Common Indian stock patterns: RELIANCE, TCS, INFY, etc.
  // US stocks typically have format: AAPL, MSFT, GOOGL
  // This is a simple heuristic - can be refined
  const indianStocks = ['RELIANCE', 'TCS', 'INFY', 'HDFC', 'ITC', 'SBIN', 'BHARTI', 'WIPRO', 'HCLTECH'];
  return indianStocks.some(s => symbol.toUpperCase().includes(s)) || 
         (!symbol.includes('.') && symbol.length <= 10 && /^[A-Z]+$/.test(symbol));
}
...
/**
 * Fetch price from NSE India (free, no API key required)
 */
async function fetchNSEPrice(symbol) {
  try {
    ...
    // Get cookies first
    await fetch(NSE_BASE, { headers }).catch(() => {});
 ...
 *  2. Try NSE India for Indian stocks, Alpha Vantage for others
 *  3. Fall back to mock price
 *
 * @param {string} symbol  - Ticker symbol (e.g. "AAPL", "RELIANCE")
...
export async function getCurrentPrice(symbol, entryPrice = null) {
  ...
  // 2. Try real API - route based on stock type
  let realPrice;
  if (isIndianStock(sym)) {
    realPrice = await fetchNSEPrice(sym);
  } else {
    realPrice = await fetchAlphaVantagePrice(sym);
  }
